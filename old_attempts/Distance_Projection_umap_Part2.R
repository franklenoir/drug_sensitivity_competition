...
### original cell lines WT expression, filtering genes just found in both. 
cell_id_orig <- c("gene","ASPC1","DU145","EFO21","NCIH1793","HCC1143","HF2597","HSTS","KRJ1","LNCAPCLONEFGC","PANC1","U87MG")
mean_exp_known <- read.delim("/export/wflenoir/dream_competition/mean_exp_known.txt") #%>% dplyr::select(-c("PANC1","HSTS","KRJ1","HF2597")) 
colnames(mean_exp_known) <- cell_id_orig
mean_exp_known <- mean_exp_known %>% filter(gene %in% ccle_dat$X)

ccle_dat <- ccle_dat %>% filter(X %in% mean_exp_known$gene)
...
umap_pipe <- function(rna_seq_dat){
...
write.csv(try_res,"./submission_v6.csv",row.names = FALSE)
